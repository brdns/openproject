...
RSpec.describe Backlogs::WorkPackageCardMenuComponent, type: :component do
  ...
  def render_component(position: 2, max_position: 3, open_sprints_exist: true, other_buckets_exist: true)
    ...
    context "when there is only one item (position=1, max=1)" do
      ...
      context "when the work package is already in the inbox" do
        ...
        it "hides the Move submenu entirely" do
          render_component(position: 1, max_position: 1, open_sprints_exist: false, other_buckets_exist: false)
