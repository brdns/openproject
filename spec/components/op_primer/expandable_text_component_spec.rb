...
RSpec.describe OpPrimer::ExpandableTextComponent, type: :component do
  ...
  describe "vertical mode" do
    ...
    it "uses flex-start alignment for vertical mode" do
      render_component(truncation: :vertical) { "Content" }

      expect(page).to have_css("div.flex-items-start")
  ...
  describe "validation" do
    it "raises for invalid truncation mode" do
      expect { render_component(truncation: :diagonal) { "Content" } }
        .to raise_error(ArgumentError, /truncation must be one of/)
    end
  end
end
